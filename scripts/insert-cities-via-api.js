...
async function main() {
  ...
  if (checkError) {
    console.log('❌ turkey_cities tablosu mevcut değil!')
    console.log('📋 Önce Supabase SQL Editor\'de şu SQL\'i çalıştırın:\n')
    console.log(`
...
DROP POLICY IF EXISTS "turkey_cities_admin_all" ON turkey_cities;
CREATE POLICY "turkey_cities_admin_all" ON turkey_cities FOR ALL USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
);
`)
    console.log('\n🔗 Supabase Dashboard: https://supabase.com/dashboard/project/cnawnprwdcfmyswqolsu/sql')
    return
  }
  ...
  const { data, error } = await supabase
    .from('turkey_cities')
    .upsert(cities, { onConflict: 'name', ignoreDuplicates: true })
    .select()

  if (error) {
    console.error('❌ Hata:', error.message)
    console.log('\n📋 Lütfen Supabase SQL Editor\'de service role için policy ekleyin:')
    console.log(`
-- Service role için tam yetki
DROP POLICY IF EXISTS "service_role_all" ON turkey_cities;
-- Aslında service role RLS'i bypass eder, bu yüzden ek policy gerekmez
`)
    return
  }

  console.log(`✅ ${cities.length} il başarıyla eklendi!`)
  ...
  console.log(`📊 Toplam ${count} il veritabanında mevcut`)
}
