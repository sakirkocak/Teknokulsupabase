...
CREATE POLICY "question_reports_student_select" ON question_reports
  FOR SELECT USING (
    student_id IN (
...
CREATE POLICY "question_reports_student_insert" ON question_reports
...
CREATE POLICY "question_reports_admin_select" ON question_reports
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "question_reports_admin_update" ON question_reports
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "question_reports_admin_delete" ON question_reports
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = auth.uid() AND role = 'admin'
...
DROP TRIGGER IF EXISTS trigger_question_report_updated_at ON question_reports;
