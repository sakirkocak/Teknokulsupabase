...
export default function MagicSearchHero() {
  ...
  const [totalQuestions, setTotalQuestions] = useState<number>(45000)  // Dinamik soru sayısı
  ...
  // Sayfa yüklendiğinde arama geçmişini yükle ve soru sayısını çek
  useEffect(() => {
    setSearchHistory(getSearchHistory())
    
    // Toplam soru sayısını Typesense'den çek
    const fetchQuestionCount = async () => {
      try {
        if (isTypesenseEnabled()) {
          const { total } = await searchQuestionsFast('*', { limit: 1 })
          if (total > 0) setTotalQuestions(total)
        }
      } catch {
        // Hata olursa varsayılan değer kullanılır
      }
    }
    fetchQuestionCount()
  }, [])
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              ...
              <span className="text-indigo-600 font-medium"> {totalQuestions.toLocaleString('tr-TR')}+ soru</span> arasında 
